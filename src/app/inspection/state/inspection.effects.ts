...
import { catchError, exhaustMap, filter, from, interval, map, of, takeUntil, tap, withLatestFrom } from 'rxjs';
...
import { cameraActions, connectivityActions, sessionActions, submissionActions, timerActions } from './inspection.actions';
import { selectCapturedPhotosList, selectRemainingSeconds, selectVehicle } from './inspection.selectors';
...
export class InspectionEffects {
    ...
    this.actions$.pipe(
      ...
      exhaustMap(() =>
        from(this.cameraService.requestAccess()).pipe(
          ...
          catchError((error: unknown) => of(cameraActions.accessDenied({ error: error as CameraAccessError }))),
        ),
      ),
    ),
  );
  ...
  readonly watchConnectivity$ = createEffect(() =>
    this.connectivityService.online$.pipe(map((online) => connectivityActions.statusChanged({ online }))),
  );
  ...
  readonly submit$ = createEffect(() =>
    this.actions$.pipe(
      ...
      exhaustMap(([, photosList, vehicle]) => {
        ...
        return this.uploadService.upload(photos, vehicle).pipe(
          map((event) => submissionActions.eventReceived({ event })),
        );
      }),
    ),
  );
