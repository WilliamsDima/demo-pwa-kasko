...
export class InspectionFacade {
  ...
  readonly status: Signal<InspectionStatus> = toSignal(this.store.select(selectStatus), {
    requireSync: true,
  });
  readonly vehicle: Signal<VehicleInfo | null> = toSignal(this.store.select(selectVehicle), {
    requireSync: true,
  });
  readonly currentStepIndex: Signal<number> = toSignal(this.store.select(selectCurrentStepIndex), {
    requireSync: true,
  });
  readonly currentStepConfig: Signal<StepConfig | null> = toSignal(
    this.store.select(selectCurrentStepConfig),
    {
      requireSync: true,
    }
  );
  ...
  readonly capturedCount: Signal<number> = toSignal(this.store.select(selectCapturedCount), {
    requireSync: true,
  });
  readonly pendingPhoto: Signal<CapturedPhotoDraft | null> = toSignal(
    this.store.select(selectPendingPhoto),
    {
      requireSync: true,
    }
  );
  readonly allRequiredCaptured: Signal<boolean> = toSignal(
    this.store.select(selectAllRequiredCaptured),
    {
      requireSync: true,
    }
  );
  ...
  readonly cameraError: Signal<CameraAccessError | null> = toSignal(
    this.store.select(selectCameraError),
    {
      requireSync: true,
    }
  );
  readonly torchOn: Signal<boolean> = toSignal(this.store.select(selectTorchOn), {
    requireSync: true,
  });
  readonly online: Signal<boolean> = toSignal(this.store.select(selectOnline), {
    requireSync: true,
  });
  readonly submission: Signal<SubmissionState> = toSignal(this.store.select(selectSubmission), {
    requireSync: true,
  });
  readonly reportAcknowledged: Signal<boolean> = toSignal(
    this.store.select(selectReportAcknowledged),
    {
      requireSync: true,
    }
  );
  ...
  submit(): void {
    this.store.dispatch(submissionActions.submitRequested({ isMock: false }));
  }

  submitMock(): void {
    this.store.dispatch(submissionActions.submitRequested({ isMock: true }));
  }
